...
generate_cassandra_versioned_docs() {
  ...
  do
    ...
    if ! git symbolic-ref -q HEAD > /dev/null; then
      ...
      converted_tags="${converted_tags} ${version}"
    fi
...
# When we generate docs, tags get converted to branches. Move them to the branches list for Antora.
if [ -n "${ANTORA_CONTENT_SOURCES_CASSANDRA_TAGS}" ]; then
  ANTORA_CONTENT_SOURCES_CASSANDRA_BRANCHES="${ANTORA_CONTENT_SOURCES_CASSANDRA_BRANCHES} ${ANTORA_CONTENT_SOURCES_CASSANDRA_TAGS}"
