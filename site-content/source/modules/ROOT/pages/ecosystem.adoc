...
https://downloads.datastax.com/#studio[DataStax Studio,window=blank]: Cross-platform (Windows, MacOSX, Linux) application that allows developers to quickly explore Apache Cassandra™ with a few clicks on their laptop, complete with tutorials and walkthroughs.

https://github.com/rustyrazorblade/easy-db-lab[easy-db-lab,window=blank]: creating lab environments for database testing in AWS.
--------
------
// end accordion
...
https://github.com/instaclustr/esop[Instaclustr Esop,window=blank]: Swiss knife for backup and restore of your node to GCP, Azure, S3, Ceph etc. Supports backup and restoration of commit logs too.
...
https://github.com/ing-bank/cassandra-jdbc-wrapper[JDBC Wrapper of the Java Drvier for Apache Cassandra,window=blank]: JDBC-compilant API for Cassandra, based on the https://github.com/apache/cassandra-java-driver[Apache Java Driver for Cassandra].
...
https://github.com/libredb/libredb-studio[LibreDB Studio,window=blank]: LibreDB Studio is an MIT licensed SQL IDE that runs in a browser and is deployed next to the database instead of installed on a workstation, shipping as a container, a Helm chart, an OpenShift operator, or an npm package embedded in another application. It connects to Cassandra over the native CQL protocol through cassandra-driver, a pure JavaScript client with no native module to install, reading the schema tree straight from system_schema. Row counts, table sizes and query plans are left out rather than estimated, since Cassandra has no honest way to produce any of the three.
...
https://github.com/apache/cassandra-sidecar[Apache Cassandra Sidecar,window=blank]: Sidecar for the highly scalable Apache Cassandra database, built as part of the Apache Cassandra project.
...
[discrete]
==== Apache Parquet
https://github.com/instaclustr/cassandra-parquet-transformer[Instaclustr Cassandra Parquet Transformer]: Transform SSTables from Apache Cassandra to Parquet or Avro files, locally or remotely via Apache Cassandra Sidecar.
...
https://github.com/apache/cassandra-analytics[Apache Cassandra Spark Bulk Reader Writer]: integration from Apache Spark directly to SSTables on disk.

https://github.com/apache/cassandra-spark-connector[Apache Cassandra Spark Connector,window=blank]: This library lets you expose Cassandra tables as Spark RDDs and Datasets/DataFrames, write Spark RDDs and Datasets/DataFrames to Cassandra tables, and execute arbitrary CQL queries in your Spark applications.
...
https://www.ibm.com/docs/en/announcements/elite-support-apache-cassandra-delivers-subscription-based-support-option-apache-cassandra-users[DataStax/IBM Elite Support for Apache Cassandra,window=blank]

https://www.ibm.com/consulting[DataStax/IBM Consulting,window=blank]
...
https://www.ksolves.com/apache-cassandra-development-company/[Ksolves Apache Cassandra Professional Services,window=blank]

https://www.ksolves.com/support-services/apache-cassandra-support/[Ksolves Apache Cassandra Support,window=blank]
...
https://datastax.academy/course/index.php?categoryid=7[DataStax/IBM Apache Cassandra 5.0 Academy,window=blank]

https://github.com/DataStax-Academy/cassandra-workshop-series[DataStax/IBM Apache Cassandra Workshops,window=blank]
