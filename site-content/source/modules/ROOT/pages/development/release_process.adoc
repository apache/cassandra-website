...
The release manager updates the development version after the vote passes. During the vote, others continue appending to the version under vote in `CHANGES.txt` and JIRA fix versions. This keeps a rollback the easy thing to do, e.g. if the vote fails.

As part of the version update:

* Compare the release branch's `CHANGES.txt` with the final release tag (`git diff <version> HEAD -- CHANGES.txt` on the release branch).
* Move entries for changes absent from the release tag into the next version's section.
* For each moved entry, update the corresponding JIRA ticket's fix version from the released version to the next version.
* Check resolved tickets for placeholder fix versions such as `5.0.x`. Replace each placeholder with the actual release version.
...
edit CHANGES.txt        # add next version section and move unreleased entries  up to it
