...
status by searching the issue tracker or looking at the
xref:development/ci.adoc[post-commit CI] results for the relevant
...
. Run the patch through xref:development/ci.adoc[pre-commit CI], with
`.build/run-ci`, and attach the `ci_summary` and `results_details`
artefacts to the ticket. This is expected of every patch before it is
committed. Contributors without access to a CI system should say so on
the ticket; the reviewer or committer then runs CI on the patch's
behalf.
