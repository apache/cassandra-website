...
The recipes for running those tests are the scripts under `.build/`, in the Cassandra repository itself; these are the same scripts CI runs.  See https://github.com/apache/cassandra/blob/trunk/.build/README.md[.build/README.md].

Running full test suites locally takes hours, if not days.  Beyond the specific tests that bear on the work at hand, rely upon the project's xref:development/ci.adoc[CI].  Push your branch to your fork, run `.build/run-ci`, and attach the resulting summary and details artefacts to the ticket.  Contributors with neither their own CI system nor an account on https://pre-ci.cassandra.apache.org[pre-ci.cassandra.apache.org] should say so on the ticket, and a committer will run CI on the patch's behalf.
...
Beginning with Cassandra 5.0, the Cassandra project is migrating from relying on ant targets for executing unit
tests, to using the `run-tests.sh` utility in `.build` : a single tool for running a wide variety of test types.
Both approaches are outlined here, but please use `run-tests.sh` when possible.

==== JUnit tests with run-test.sh

See https://github.com/apache/cassandra/blob/trunk/.build/README.md#running-tests[.build/README] for complete
...
This is probably not what you want to do though, since this command will run all the unit tests
...
The `TestClassNameRegex` parameter can be a simple or fully qualified class name, or a regular expression. For example:
...
Again, this is probably not what you want to do, since this will run all the unit tests
...
The `TestClassName` property can be either a simple or fully qualified class name.

To run a specific test in a test class, use the same command with an additional `test.methods` property:
...
You can specify multiple methods in a class by separating the method names by comma.
...
the project's xref:development/ci.adoc[CI], which runs the very same
image and scripts.

The simplest route is to let the wrapper script do the work; it builds
or pulls the image, mounts your working tree, and runs the given test
target:

[source,plaintext]
----
.build/docker/run-tests.sh -a dtest
.build/docker/run-tests.sh -a dtest -t "test_bootstrap"
----

The rest of this section covers working inside the container by hand,
which is useful when iterating on a single test.
